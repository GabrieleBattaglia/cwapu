    ...
    if current_aggregates['aggregated_errors_detail']:
        ...
        if error_counts:
            ...
            for i, bar_patch in enumerate(bars):
                ...
                annotation_text = _(" {count} ({perc_vs_total}% dei car. tot., {perc_vs_specific}% delle '{char_upper}' inviate)").format(count=count, perc_vs_total=perc_vs_total, perc_vs_specific=perc_vs_specific)
...
def Rxing():
        ...
        if needs_processing:
            ...
            if original_qrz == guess:
                ...
                if not fix_speed and overall_speed > 15:
                    overall_speed -= 1
            calls += 1
            if overall_speed > maxwpm:
                maxwpm = overall_speed
            if overall_speed < minwpm:
                minwpm = overall_speed
            repeatedflag = False
    ...
    if len(callssend) >= 10:
        ...
        print(_('Durante la sessione, la tua velocità minima è stata {minwpm}, la massima di {maxwpm}: pari ad una variazione di {range_wpm} WPM.\n\tLa velocità media di ricezione è di: {average_wpm:.1f} WPM.').format(minwpm=minwpm, maxwpm=maxwpm, range_wpm=maxwpm - minwpm, average_wpm=average_rwpm / len(callsget)))
            ...
            for tag, i1, i2, j1, j2 in s.get_opcodes():
                if tag == 'replace':
                    ...
                    for char in received_str[j1:j2]:
                        char_error_counts[char] = char_error_counts.get(char, 0) + 1
                        total_mistakes_calculated += 1
        print(_('Carattere: errori = Percentuale'))
        if total_mistakes_calculated > 0:
            ...
            for char, count in sorted_errors:
                ...
                print(_("   '{char_display}': {count} ({percentage_vs_total_session_chars:.1f}% dei car. tot., {percentage_vs_this_char_sent_session:.1f}% delle '{char_display}' inviate)").format(char_display=char.upper(), count=count, percentage_vs_total_session_chars=percentage_vs_total_session_chars, percentage_vs_this_char_sent_session=percentage_vs_this_char_sent_session))
        ...
        f.write(_('Durante la sessione, la tua velocità minima è stata {minwpm}, la massima di {maxwpm}: pari ad una variazione di {range_wpm} WPM.\n\tLa velocità media di ricezione è di: {average_wpm:.1f} WPM.').format(minwpm=minwpm, maxwpm=maxwpm, range_wpm=maxwpm - minwpm, average_wpm=avg_wpm_calc) + '\n')
        f.write(_('Carattere: errori = Percentuale'))
        if total_mistakes_calculated > 0:
            for char, count in sorted_errors:
                percentage_vs_total_session_chars = count / send_char * 100 if send_char > 0 else 0.0
                total_sent_of_this_char_session = sent_chars_detail_this_session.get(char, 0)
                percentage_vs_this_char_sent_session = count / total_sent_of_this_char_session * 100 if total_sent_of_this_char_session > 0 else 0.0
                f.write(_("\n    '{char_display}': {count} ({percentage_vs_total_session_chars:.1f}% dei car. tot., {percentage_vs_this_char_sent_session:.1f}% delle '{char_display}' inviate)").format(char_display=char.upper(), count=count, percentage_vs_total_session_chars=percentage_vs_total_session_chars, percentage_vs_this_char_sent_session=percentage_vs_this_char_sent_session))
            f.write('\n')
            f.write(_('\nErrori totali: {global_mistakes} su {send_char} = {mistake_percentage:.2f}%').format(global_mistakes=total_mistakes_calculated, send_char=send_char, mistake_percentage=mistake_percentage))
            f.write(_('\nCaratteri mai sbagliati: {good_letters}').format(good_letters=' '.join(sorted(good_letters)).upper()))
        else:
    ...
    try:
        with open(report_filename_full_path, 'w', encoding='utf-8') as f:
            ...
            if current_aggregates.get('aggregated_errors_detail', {}):
                ...
                f.write('<table>\n')
                f.write(_('  <thead><tr><th>Carattere</th><th>N. Errori</th><th>% su Tot. Caratt.</th><th>% su Caratt. Spec. Inv.</th></tr></thead>\n'))
                f.write('  <tbody>\n')
                ...
                for char, count in sorted_errors:
                    ...
                    f.write(_('    <tr><td class="char-emphasis">\'{}\'</td><td>{count}</td><td>{percentage_vs_total_block_chars}%</td><td>{percentage_vs_this_char_sent}% <span class="details-label">(su {count} \'{char_upper}\' inv.)</span></td></tr>\n').format(char.upper(), count=count, percentage_vs_total_block_chars=percentage_vs_total_block_chars, percentage_vs_this_char_sent=percentage_vs_this_char_sent))
                f.write('  </tbody>\n</table>\n')
