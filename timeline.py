...
def genera_report_attivita(df: pd.DataFrame, _, lang, giorni, mesi) -> str:
    ...
    def _formatta_delta_temporale(delta, _):
        ...
        if anni > 0:
            parti.append(_("{} anni").format(anni))
        if mesi > 0:
            parti.append(_("{} mesi").format(mesi))
        if giorni_finali > 0:
            parti.append(_("{} giorni").format(giorni_finali))
        if ore > 0:
            parti.append(_("{} ore").format(ore))
        if minuti > 0:
            parti.append(_("{} minuti").format(minuti))
    ...
    hourly_stats = df.groupby(df["timestamp_iso"].dt.hour).agg(count=("timestamp_iso", "size"), total_duration=("duration_seconds", "sum"))
    ...
    for hour, stats in hourly_stats.sort_values(by="total_duration", ascending=False).iterrows():
        ...
        report_lines.append(
            _("   - Fascia {start_hour:02d}-{end_hour:02d}: {count} sessioni ({percentage:.2f}%), durata {durata}").format(
    ...
    fasce_6h_stats = df_copy.groupby("fascia_6h").agg(count=("timestamp_iso", "size"), total_duration=("duration_seconds", "sum"))
    for fascia, stats in fasce_6h_stats.sort_values(by="total_duration", ascending=False).iterrows():
        percentage = (stats["total_duration"] / total_duration_overall) * 100 if total_duration_overall > 0 else 0
        durata_str = _formatta_durata(stats["total_duration"], _)
        report_lines.append(_("   - Fascia {fascia}: {count} sessioni ({percentage:.2f}%), durata {durata}").format(fascia=fascia, count=int(stats["count"]), percentage=percentage, durata=durata_str))
    report_lines.append(_("Distribuzione Settimanale"))
    weekly_stats = df.groupby(df["timestamp_iso"].dt.dayofweek).agg(count=("timestamp_iso", "size"), total_duration=("duration_seconds", "sum"))
    ...
    for day_idx, stats in weekly_stats.sort_values(by="total_duration", ascending=False).iterrows():
        ...
        durata_str = _formatta_durata(stats["total_duration"], _)
        report_lines.append(
            _("   - {giorno}: {count} sessioni ({percentage:.2f}%), durata {durata}").format(giorno=giorni[day_idx], count=int(stats["count"]), percentage=percentage, durata=durata_str)
        )
    ...
    monthly_stats = df.groupby(df["timestamp_iso"].dt.month).agg(count=("timestamp_iso", "size"), total_duration=("duration_seconds", "sum"))
    ...
    for month_idx, stats in monthly_stats.sort_values(by="total_duration", ascending=False).iterrows():
        percentage = (stats["total_duration"] / total_duration_overall) * 100 if total_duration_overall > 0 else 0
        durata_str = _formatta_durata(stats["total_duration"], _)
        report_lines.append(_("   - {mese}: {count} sessioni ({percentage:.2f}%), durata {durata}").format(mese=mesi[month_idx], count=int(stats["count"]), percentage=percentage, durata=durata_str))
    ...
    seasonal_stats = df_copy.groupby("stagione").agg(count=("timestamp_iso", "size"), total_duration=("duration_seconds", "sum"))
    for stagione, stats in seasonal_stats.sort_values(by="total_duration", ascending=False).iterrows():
        percentage = (stats["total_duration"] / total_duration_overall) * 100 if total_duration_overall > 0 else 0
        durata_str = _formatta_durata(stats["total_duration"], _)
        report_lines.append(_("   - {stagione}: {count} sessioni ({percentage:.2f}%), durata {durata}").format(stagione=stagione, count=int(stats["count"]), percentage=percentage, durata=durata_str))
    return "\n".join(report_lines)
...
def genera_report_performance(df: pd.DataFrame, _, lang) -> str:
    ...
    stat_caratteri = []
    for _indice, sessione in df.iterrows():
    ...
    if not stat_caratteri:
        ...
        if char_df_filtrato.empty:
            ...
            for char, row in caratteri_peggiori.iterrows():
                report_lines.append(
                    _("  - Carattere '{}': Errore stimato {:.2f}% ±{:.2f}% ({} errori su {} invii)").format(char, row["media_errore"], row["incertezza"], int(row["errori"]), int(row["inviati"]))
                )
            if not caratteri_peggiori.empty:
                ...
                for _indice, sessione in prima_meta_df.iterrows():
                ...
                for _indice, sessione in seconda_meta_df.iterrows():
...
def genera_report_velocita_avanzato(df: pd.DataFrame, _, lang) -> str:
    ...
    report_lines.append(_("  - (La meta' delle tue sessioni ha una velocità media tra {:.2f} e {:.2f} WPM)").format(q1, q3))
...
def genera_report_stagionale(df: pd.DataFrame, _, lang) -> str:
    ...
    if "accuracy" not in df_copy.columns:
        df_copy["accuracy"] = np.where(df_copy["items_sent_session"] > 0, (df_copy["items_correct_session"] / df_copy["items_sent_session"]) * 100, 0)
    # Raggruppa per stagione e calcola i valori massimi
    stat_stagionali = df_copy.groupby("stagione").agg(picco_wpm=("rwpm_max", "max"), picco_accuratezza=("accuracy", "max"))
    ...
    if not stat_stagionali.empty:
        ...
        if stagione_top_wpm == stagione_top_acc:
            report_lines.append(
                _("  Hai raggiunto i tuoi picchi sia di velocità ({:.0f} WPM) che di accuratezza ({:.2f}%) durante la stagione: {}.").format(
                    stat_stagionali["picco_wpm"].max(), stat_stagionali["picco_accuratezza"].max(), stagione_top_wpm
                )
            )
        else:
            report_lines.append(
                _("  La tua velocità di punta ({:.0f} WPM) è stata registrata in {}, mentre hai raggiunto la massima accuratezza ({:.2f}%) in {}.").format(
                    stat_stagionali["picco_wpm"].max(), stagione_top_wpm, stat_stagionali["picco_accuratezza"].max(), stagione_top_acc
                )
            )
    return "\n".join(report_lines)
...
def genera_report_quartili(df: pd.DataFrame, _, lang, giorni, mesi) -> str:
    ...
    global_errors = {}
    global_sents = {}
    for _indice, sessione in df_copy.iterrows():
...
def genera_report_temporale_completo(dati_sessioni, _, lang) -> str:
    ...
    return header + report_attivita + report_performance + report_velocita_avanzato + report_avanzato + report_confronto + report_stagionale + report_quartili
